...
!
! !DESCRIPTION:
!  Here, the NflexPD_phy_Cbased namelist is read and variables exported
!  by the model are registered with FABM.
!
! !INPUT PARAMETERS:
   class (type_NflexPD_phy_Cbased), intent(inout), target :: self
...
!
! !INPUT PARAMETERS:
   class (type_NflexPD_phy_Cbased), intent(in) :: self
...
!BOC
   ...
   ! Rertrieve current (local) state variable values     
  ...
  ! Retrieve current environmental conditions.
   ...
   !  write(*,'(A,F6.1,4F16.10)')'  (phyL414) doy,parE_dm,aim,LamW,ThetaHat:',doy,parE_dm*secs_pr_day,aim,LamW,ThetaHat
   !end if
   ...
   if (.not. self%dynQN) then
     if ( self%mimic_Monod ) then
       ...
       ![-] = molC/molN + s-1/(molN/molC/s) * molN/molC
       ...
       if (delta_temp .eq. 0.0) then
         ...
         if (self%theta_opt) then
           ...
           if( parE_dm .gt. I_zero ) then
             ...
             LamW_p = FLamW(aim_p,Ld,mu0hat_fT_p,RMchl_fT_p,zetaChl)
             ThetaHat_p = 1.0/zetaChl + (1.0 - LamW_p)/aim_p
           else
   ...
   if ( self%dynQN .or. self%mimic_Monod) then
     ...
     _SET_DIAGNOSTIC_(self%id_vN_dQdt_I,(vN+delQ_delI*dI_dt+delQ_delLd*dLd_dt+delQ_delTemp*dTemp_dt)*phyC*secs_pr_day) !mmolN/m^3/d
     !RHS's are set externallly
